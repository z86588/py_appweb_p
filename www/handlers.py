...
async def api_register_user(*, email, name, passwd):
    ...
    email_sha = hashlib.md5(email.encode('utf-8')).hexdigest()
    logging.info('>>>>>image param: %s' % email_sha)
    user = User(id=uid, name=name.strip(), email=email, passwd=hashlib.sha1(sha1_passwd.encode('utf-8')).hexdigest(),
                image='http://www.gravatar.com/avatar/%s?d=mm&s=120' % email_sha)
    await user.save()
...
async def cookie2user(cookie_str):
    ...
    try:
        ...
        if user is None:
            return None
        s = '%s-%s-%s-%s' % (uid, user.passwd, expire, _COOKIE_KEY)
